...
/// Resolve `~/.cache/orrery-tui/horizons/` (via `directories` crate).
#[must_use]
pub fn cache_root() -> PathBuf {
    ProjectDirs::from("dev", "orrery-tui", "orrery-tui")
        .map(|dirs| dirs.cache_dir().join("horizons"))
        .unwrap_or_else(|| PathBuf::from(".cache/orrery-tui/horizons"))
}
...
pub fn read_raw_cache(path: &Path) -> Result<HorizonsResponse> {
    let text = fs::read_to_string(path).map_err(|e| OrreryTuiError::Io {
        path: path.to_path_buf(),
        source: e,
    })?;
    serde_json::from_str(&text).map_err(|e| {
        OrreryTuiError::Horizons(crate::error::HorizonsError::Http(format!(
...
pub fn write_raw_cache(path: &Path, response: &HorizonsResponse) -> Result<()> {
    ...
    let json = serde_json::to_string_pretty(response)
        .map_err(|e| OrreryTuiError::Horizons(crate::error::HorizonsError::Http(e.to_string())))?;
    fs::write(path, json).map_err(|e| OrreryTuiError::Io {
        path: path.to_path_buf(),
        source: e,
    })
}
